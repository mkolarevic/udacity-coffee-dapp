...
contract('SupplyChain', function (accounts) {
    ...
    // Custom test to assign roles
    it("all roles are added correctly", async () => {
        const supplyChain = await SupplyChain.deployed()
        let isFarmer = await supplyChain.isFarmer(originFarmerID)
        assert.equal(isFarmer, false, 'Error: Farmer is assigned when it shouldn\'t be')
        let isDistributor = await supplyChain.isDistributor(distributorID)
        assert.equal(isDistributor, false, 'Error: Distributor is assigned when it shouldn\'t be')
        let isRetailer = await supplyChain.isRetailer(retailerID)
        assert.equal(isRetailer, false, 'Error: Retailer is assigned when it shouldn\'t be')
        let isConsumer = await supplyChain.isConsumer(consumerID)
        assert.equal(isConsumer, false, 'Error: Consumer is assigned when it shouldn\'t be')
        await supplyChain.addFarmer(originFarmerID)
        await supplyChain.addDistributor(distributorID)
        await supplyChain.addRetailer(retailerID)
        await supplyChain.addConsumer(consumerID)
        isFarmer = await supplyChain.isFarmer(originFarmerID)
        assert.equal(isFarmer, true, 'Error: Farmer is assigned when it shouldn\'t be')
        isDistributor = await supplyChain.isDistributor(distributorID)
        assert.equal(isDistributor, true, 'Error: Distributor is assigned when it shouldn\'t be')
        isRetailer = await supplyChain.isRetailer(retailerID)
        assert.equal(isRetailer, true, 'Error: Retailer is assigned when it shouldn\'t be')
        isConsumer = await supplyChain.isConsumer(consumerID)
        assert.equal(isConsumer, true, 'Error: Consumer is assigned when it shouldn\'t be')
    })
    ...
    it("Testing smart contract function harvestItem() that allows a farmer to harvest coffee", async () => {
        ...
        /* this would be the syntax for older versions of truffle
        let event = supplyChain.Harvested()
        await event.watch((err, res) => {
            eventEmitted = true
        })
        */
        let event = supplyChain.contract.events.Harvested
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function processItem() that allows a farmer to process coffee", async () => {
        ...
        let event = supplyChain.contract.events.Processed
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function packItem() that allows a farmer to pack coffee", async () => {
        ...
        let event = supplyChain.contract.events.Packed
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async () => {
        ...
        let event = supplyChain.contract.events.ForSale
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async () => {
        ...
        let event = supplyChain.contract.events.Sold
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function shipItem() that allows a distributor to ship coffee", async () => {
        ...
        let event = supplyChain.contract.events.Shipped
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function receiveItem() that allows a retailer to mark coffee received", async () => {
        ...
        let event = supplyChain.contract.events.Received
        await event((err, res) => {
            eventEmitted = true
        })
    ...
    it("Testing smart contract function purchaseItem() that allows a consumer to purchase coffee", async () => {
        ...
        let event = supplyChain.contract.events.Purchased
        await event((err, res) => {
            eventEmitted = true
        })
